...
- `_boom_hunter(closes)` → `(trigger, q2, q3, q4, q5, q6)` — Three EOT instances: EOT1 (lpperiod=6, fast), EOT2 (lpperiod=27, trend), EOT3 (lpperiod=11, extreme).
...
**Parameters to sweep:**
- EOT1 lpperiod: 4, 5, 6, 7, 8 (currently 6)
...
1. Use EOT3 extreme detector as an EXIT filter (tighten ATR stop when EOT3 is overbought/oversold)
2. Use SDO as a trend-quality filter (only enter when SDO is between 30-70, meaning not at extremes)
3. Add RSI divergence as a 7th signal with MIN_VOTES still at 4 (additive, not replacing)
4. Use EOT2 convergence width as a volatility filter (like BB compression but DSP-based)
5. Combine BB compression AND EOT2 convergence for a dual-compression signal
6. Use SDO slope (current - previous) as momentum confirmation instead of raw returns
7. Replace MACD with EOT1 trigger-Q2 spread (both measure trend strength differently)
8. Use _ehlers_eot with different lpperiods (try 4, 8, 12) as additional entry/exit filters
